...
class Product(BaseModel):
    ...
    # default_factory=uuid4: Product Class가 처음 만들어질 때 호출되는 함수를 uuid4로 하겠다. => Product 클래스를 생성하면 uuid4를 만들어서 id에 저장한다.
    name: str
    price: float
...
class Order(BaseModel):
    ...
    # default_factory=list: 최초에 빈 list를 만들어서 저장한다.
    ...
    def add_product(self, product: Product):
        # Product를 인자로 받아서 해당 id가 이미 존재하는 지 체크 -> 없다면 products 필드에 추가
        if product.id in [existing_product.id for existing_product in self.products]:
            return self
        self.products.append(product)
        # 업데이트할 때 updated_at을 현재 시작으로 업데이트
...
async def get_order(order_id: UUID) -> Union[Order, dict]:
    ...
    # 만약, get_order_by_id에서 빈 리스트가 나오면? -> 예외 처리
    if not order:
...
async def make_order(files: List[UploadFile] = File(...),
    ...
    # Depends: 의존성 주입 함수, 반복적이고 공통적인 로직이 필요할 때(ex. 모델 load, config load) 사용할 수 있음
    # async, Depends 검색해서 학습해보기
    products = []
    for file in files:
        image_bytes = await file.read()
        inference_result = predict_from_image_byte(model=model, image_bytes=image_bytes, config=config)
        # InferenceImageProduct Class를 생성해서 product로 정의
...
def update_order_by_id(order_id: UUID, order_update: OrderUpdate) -> Optional[Order]:
    ...
    updated_order = update_order.copy()
